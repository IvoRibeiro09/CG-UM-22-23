...
int drawBox(int length, int side, char* filepath) 
{
    ...
    // baixo
            ...
            float xTex1 = (1.0f / (float)side) * iterX;
            float yTex1 = (1.0f / (float)side) * j;
        
            float xTex3 = (1.0f / (float)side) * iterX;
            float yTex3 = (1.0f / (float)side) * (iterY+1);

            float xTex2 = (1.0f / (float)side) * (iterX+1);
            float yTex2 = (1.0f / (float)side) * (iterY);

            float xTex4 = (1.0f / (float)side) * (iterX+1);
            float yTex4 = (1.0f / (float)side) * (iterY+1);

            float xTex5 = (1.0f / (float)side) * (iterX+1);
            float yTex5 = (1.0f / (float)side) * iterY;

            float xTex6 = (1.0f / (float)side) * iterX;
            float yTex6 = (1.0f / (float)side) * (iterY+1);
...
void drawPlane(int length, int divisions,char*filepath)
{
	...
	if (divisions % 2 != 0) 
	{
		...
		for (int j = -(divisions / 2); j <= (divisions / 2); j++) 
		{
			...
			{
                float xTex4 = (1.0f / (float)divisions) * iterX;
                float yTex4 = (1.0f / (float)divisions) * iterY;

                float xTex5 = (1.0f / (float)divisions) * (iterX+1);
                float yTex5 = (1.0f / (float)divisions) * (iterY+1);

                float xTex6 = (1.0f / (float)divisions) * iterX;
                float yTex6 = (1.0f / (float)divisions) * (iterY+1);

                float xTex1 = (1.0f / (float)divisions) * iterX;
                float yTex1 = (1.0f / (float)divisions) * iterY;

                float xTex2 = (1.0f / (float)divisions) * (iterX+1);
                float yTex2 = (1.0f / (float)divisions) * iterY;

                float xTex3 = (1.0f / (float)divisions) * (iterX+1);
                float yTex3 = (1.0f / (float)divisions) * (iterY+1);


				file << (-0.5 + i) * length<< " "<< 0.0f<<" " <<(0.5 + j) * length <<std::endl;
                file << xTex1 << " "<< yTex1 <<std::endl;
                file << (-0.5 + i) << " "<< 0.0f<<" " <<(0.5 + j) <<std::endl;
				file << (0.5 + i) * length<< " "<< 0.0f<<" " <<(0.5 + j) * length <<std::endl;
                file << xTex2 << " "<< yTex2 <<std::endl;
                file << (0.5 + i) << " "<< 0.0f<<" " <<(0.5 + j) <<std::endl;
				file << (0.5 + i) * length<< " "<< 0.0f<<" " <<(-0.5 + j) * length <<std::endl;
...
int main(int argc,char**argv){
    ...
    vector<float> pontos, normais, texture;
    if(opcao == 7){
        float altura, raio, lados;
        printf("Altura:");
        scanf("%f", &altura);
        printf("Raio:");
        scanf("%f", &raio);
        ...
        cilindro(&pontos, &normais, &texture, altura, raio, lados);
        float arr[3] = {altura, raio, lados};
        vectorTo3dFile(pontos, normais, texture, generateChar("Cilinder",arr , 3));
    }
